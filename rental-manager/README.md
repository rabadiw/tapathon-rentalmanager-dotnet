# Steeltoe RentalManager Accelerator
...
```
tanzu apps workload create rental-manager -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/rental-manager-source \
  --type web
```
...
```
tanzu apps workload get rental-manager
```
...
```
dotnet publish -o publish RentalManager.csproj
```

Push the app to Cloud Foundry:

```
cf push rental-manager -p ./publish --random-route
```
